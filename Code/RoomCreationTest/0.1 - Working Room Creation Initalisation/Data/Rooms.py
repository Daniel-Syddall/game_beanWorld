    ...
    for r in range(len(temp_fileNames)):
        ...
        for s in range(room[temp_roomName]["sendCount"]):
            ...
            for d in range(len(temp_lineStore)-1):
                temp_varStore = temp_lineStore[d]
                temp_varStore = temp_varStore.split("-")
                if temp_varStore == "sendX": room[temp_roomName][temp_varStore[0]] = int(temp_varStore[1])
                elif temp_varStore == "sendX": room[temp_roomName][temp_varStore[0]] = int(temp_varStore[1])
                elif temp_varStore == "sendRoom": room[temp_roomName][temp_varStore[0]] = temp_varStore[1]
                elif temp_varStore == "sendSpawn": room[temp_roomName][temp_varStore[0]] = int(temp_varStore[1])
