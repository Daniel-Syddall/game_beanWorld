...
def Room_SetGround(self,RoomName,StartX,StartY,EndX,EndY,Ground1,Ground2):
    ...
    for y in range((HighY-LowY-1)): self.room[RoomName][f"2_{str(HighX)}_{str(y+1+LowY)}"] = "tile_ground_"+Ground2+"_right"
    for x in range((HighX-LowX-1)): self.room[RoomName][f"2_{str(x+1+LowX)}_{str(HighY)}"] = "tile_ground_"+Ground2+"_down"
    for y in range((HighY-LowY-1)): self.room[RoomName][f"2_{str(LowX)}_{str(y+1+LowY)}"] = "tile_ground_"+Ground2+"_left"
    for x in range((HighX-LowX)-1):
...
def Room_SetEdge(self,RoomName,StartX,StartY,EndX,EndY,Edge,Ground):
    ...
    for y in range((HighY-LowY-1)): self.room[RoomName][f"1_{str(HighX)}_{str(y+1+LowY)}"] = "tile_edge_"+Edge+"_right"
    for x in range((HighX-LowX-1)): self.room[RoomName][f"1_{str(x+1+LowX)}_{str(HighY)}"] = "tile_edge_"+Edge+"_down-0"
    for y in range((HighY-LowY-1)): self.room[RoomName][f"1_{str(LowX)}_{str(y+1+LowY)}"] = "tile_edge_"+Edge+"_left"
    ...
    for y in range((HighY-LowY-1)): self.room[RoomName][f"2_{str(HighX)}_{str(y+1+LowY)}"] = "tile_ground_"+Ground+"_right"
    for x in range((HighX-LowX-1)): self.room[RoomName][f"2_{str(x+1+LowX)}_{str(HighY)}"] = "tile_ground_"+Ground+"_down"
    for y in range((HighY-LowY-1)): self.room[RoomName][f"2_{str(LowX)}_{str(y+1+LowY)}"] = "tile_ground_"+Ground+"_left"
    for x in range((HighX-LowX)-1):
...
def Room_ChangeHue(self,RoomName,layer,StartX,StartY,EndX,EndY,hue):
    LowX,HighX,LowY,HighY = Room_GetLowAndHigh(StartX,StartY,EndX,EndY)
    hue = hue.split("_")
    for x in range((HighX-LowX)+1):
        for y in range((HighY-LowY)+1):
            tile = self.room[RoomName][f"{str(layer)}_{str(LowX+x)}_{str(LowY+y)}"]
            if tile != "" and tile[0] != "shadow":
                tile = tile.split("_")
                if tile[0] == "sprite" or tile[0] == "tile" or tile[0] == "rgba" or tile[0] == "pco":
                    if tile[0] == "rgba": hueStart = 1
                    elif tile[0] == "pco" or tile[0] == "overlay": hueStart = 2
                    elif tile[1] == "decor": hueStart = 3
                    else: hueStart = 4
                    if len(tile) != hueStart+4: 
                        for i in range(4): tile.append(str(hue[i]))
                    else: 
                        for i in range(4): tile[i+hueStart] = str(int(tile[i+hueStart]) + int(hue[i]))
                    for i in range(4):
                        if tile[0] == "rgba" and int(tile[i+hueStart]) > 255: tile[i+hueStart] = "255"
                        elif tile[0] == "rgba" and int(tile[i+hueStart]) < -255: tile[i+hueStart] = "0"
                    newTile = ""
                    for i in range(len(tile)): newTile += tile[i] + "_"
                    newTile = newTile[:-1]
                    self.room[RoomName][f"{str(layer)}_{str(LowX+x)}_{str(LowY+y)}"] = newTile
...
def Room_CreateSend(self,SendRoom,SendX,SendY,SpawnRoom,SpawnX,SpawnY):
    SendID = self.room[SendRoom]["sendCount"]
