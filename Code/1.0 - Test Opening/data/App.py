...
class Map:
    ...
    def blit(self):
        temp_surfaceX,temp_surfaceY,temp_surfaceWidth,temp_surfaceHeight = rpg.Blit(self.app.window,math.floor((self.app.w(100)-((self.app.height/13)*17))/2),self.app.h(0),(self.app.height/13)*17,self.app.height)

        if rpg.textBox != "":
            if rpg.asset["character"][rpg.textBox_character] == None:
                hue = rpg.textBox_character
                hue = hue.split("~")
                rpg.asset["character"][rpg.textBox_character] = pygame.image.load(f"{self.local}/assets/characters/{hue[0]}.png")
                temp_area = pygame.PixelArray(rpg.asset["character"][rpg.textBox_character])
                for X in range(rpg.asset["character"][rpg.textBox_character].get_width()):
                    for Y in range(rpg.asset["character"][rpg.textBox_character].get_height()):
                        temp_rgb = rpg.asset["character"][rpg.textBox_character].unmap_rgb(temp_area[X][Y])
                        temp_color = pygame.Color(*temp_rgb)
                        R,G,B,A = temp_color
                        if A != 0:
                            temp_rgba = []
                            temp_rgba.append(int(R)+int(hue[1]))
                            temp_rgba.append(int(G)+int(hue[2]))
                            temp_rgba.append(int(B)+int(hue[3]))
                            temp_rgba.append(int(A)+int(hue[4]))
                            for i in range(4):
            ...
            startX = temp_surfaceX + (temp_surfaceWidth*(0.052))
            startY = temp_surfaceY + (temp_surfaceHeight*(0.79))
            maxWidth = math.floor(temp_surfaceWidth*(0.12))
            maxHeight = math.floor(temp_surfaceHeight*(0.2))
            if rpg.asset["character"][rpg.textBox_character].get_width() >= rpg.asset["character"][rpg.textBox_character].get_height():
                imageWidth = maxWidth
            ...
            self.app.window.blit(pygame.transform.scale(rpg.asset["character"][rpg.textBox_character],(imageWidth,imageHeight)),(imageX,imageY))
