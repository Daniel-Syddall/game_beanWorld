...
def Room_ChangeLayerHue(self,RoomName,layer,hue):
    hue = hue.split("_")
    for x in range(self.room[RoomName]["maxX"]):
        for y in range(self.room[RoomName]["maxY"]):
            tile = self.room[RoomName][f"{str(layer)}_{str(x)}_{str(y)}"]
            if tile != "" and tile[0] != "shadow":
                ...
                if tile[0] == "sprite" or tile[0] == "tile" or tile[0] == "rgba" or tile[0] == "pco":
                    ...
                    for i in range(4):
                        if int(tile[i+hueStart]) > 255: tile[i+hueStart] = "255"
                        elif int(tile[i+hueStart]) < 0: tile[i+hueStart] = "0"
                    newTile = ""
                    for i in range(len(tile)): newTile += tile[i] + "_"
                    newTile = newTile[:-1]
                    self.room[RoomName][f"{str(layer)}_{str(x)}_{str(y)}"] = newTile
